...
public:
	...
	void ResetViewPort();
	bool Blit(SDL_Texture* texture, int x, int y, const SDL_Rect* section = NULL, bool fliped = false, bool cameraUse = true, Uint8 alpha = 0, Uint8 r = 255, Uint8 g = 255, Uint8 b = 255, float scale = 1.0f, float pivotX = 0, float pivotY = 0, float speedX = 1.0f, float speedY = 1.0f, double angle = 0, int pivot_x = INT_MAX, int pivot_y = INT_MAX);
	bool MinimapBlit(SDL_Texture* texture, int x, int y, const SDL_Rect* section, float scale) const;
